...
def evaluateTest(pairs, encoder, decoder, searcher, voc):
    ...
    for (input_sentence, output_sentence) in new_pair:
        ...
        # for end_word in beam_output_words:
        #     beam_output_sentence = beam2tokens(end_word, voc)
        #     print(beam_output_sentence)
        beam_output_words = beam_evaluate(encoder, decoder, searcher, voc, input_sentence)
        beam_score = np.max([get_bleu_result(beam2tokens(end_word, voc), [output_sentence]) for end_word in beam_output_words])
...
def beam_evaluate(encoder, decoder, searcher, voc, sentence, beam_width = 3, max_length = MAX_LENGTH):
    ### Format input sentence as a batch
    # words -> indexes
    indexes_batch = [indexesFromSentence(voc, sentence)]
    # Create lengths tensor
    ...
    while not q.empty():
            ...
            for _ in range(q.qsize()):
                ...
                length = node.length
                # 搜索终止条件
                if decoder_input.item() == EOS_token or node.length >= max_length:
